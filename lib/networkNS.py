...
class NetComNS_InN_legendre(nn.Module):
    ...
    def __init__(self, num_blocks, Params = None, cheb = False):
        ...
        if cheb:
            file_path = str(module_dir / 'chebyshevs' / 'ChebyshevConv{}.mat')
            print("*"*5, 'using Chebyshev filters', "*"*5)
        else:
            file_path = str(module_dir / 'legendres' / 'LegendreConv{}.mat')
            print("*"*5,'using Legendre filters',"*"*5)
        with_bias = False  # True#

        filter_d, filter_r, self.l_filters = generte_legendre_filters_2D(
            file_path,
            n=self.n,
            m=self.m,
        )
        self.register_buffer("filter_d", filter_d, persistent=False)
        self.register_buffer("filter_r", filter_r, persistent=False)
